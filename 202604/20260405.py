...
class Solution:
    ...
    def judge_circle(self, moves: str) -> bool:
        # Initialize position coordinates at origin
        x = 0
        y = 0
        for move in moves:
            # Update position for each valid move
            if move == "R":
                x += 1
            elif move == "L":
                x -= 1
                ...
                y -= 1
        # Return true only if net displacement is zero in both axes
        return x == 0 and y == 0

    judgeCircle = judge_circle