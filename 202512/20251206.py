...
class Solution:
    ...
    def count_partitions(self, nums: list[int], k: int) -> int:
        ...
        for right_index, current_number in enumerate(nums):
            ...
            while (
                max_value_index_queue
                and current_number > nums[max_value_index_queue[-1]]
            ):
            ...
            while (
                min_value_index_queue
                and current_number < nums[min_value_index_queue[-1]]
            ):
            ...
            while (
                max_value_index_queue
                and min_value_index_queue
                and nums[max_value_index_queue[0]] - nums[min_value_index_queue[0]] > k
            ):
                # Subtract ways starting at old left_pointer (cannot extend to right_index)
                total_ways_count = (
                    total_ways_count - ways_dynamic_programming[left_pointer] + MOD
                ) % MOD
                left_pointer += 1
