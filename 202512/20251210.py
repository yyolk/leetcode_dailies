...
class Solution:
    ...
    def count_permutations(self, complexity: list[int]) -> int:
        ...
        MOD = 10**9 + 7
        n = len(complexity)
        # Check if complexity[0] is strictly less than all complexity[i] for i >= 1
        # If not, impossible to unlock all, return 0
        for i in range(1, n):
            if complexity[i] <= complexity[0]:
                return 0
        # Otherwise, all can be unlocked in any order after 0, so (n-1)! % MOD
        res = 1
        for i in range(1, n):
            res = (res * i) % MOD
        return res

    countPermutations = count_permutations