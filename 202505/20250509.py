...
class Solution:
    ...
    def count_balanced_permutations(self, num: str) -> int:
        ...
        number_of_odd_positions = total_length - total_length // 2  # Ceiling division for odd positions
        number_of_even_positions = total_length // 2  # Floor division for even positions
        ...
        @cache
        def compute_permutations(current_digit: int, remaining_odd_positions: int, 
                               remaining_even_positions: int, remaining_balance: int) -> int:
            # Base case: all positions are filled and the balance is zero (valid permutation)
            if remaining_odd_positions == 0 and remaining_even_positions == 0 and remaining_balance == 0:
                return 1  # Found a valid balanced permutation
            
            # Invalid state: negative digit, positions, or balance
            if (current_digit < 0 or remaining_odd_positions < 0 or 
                remaining_even_positions < 0 or remaining_balance < 0):
            ...
            for odd_position_count in range(0, digit_frequency_counter[current_digit] + 1):
                # Calculate remaining digits to assign to even positions
                even_position_count = digit_frequency_counter[current_digit] - odd_position_count
                ...
                ways_to_choose_even = comb(remaining_even_positions, even_position_count)
                ...
                recursive_result = compute_permutations(
                    ...
                    remaining_odd_positions - odd_position_count,  # Reduce odd positions used
                    remaining_even_positions - even_position_count,  # Reduce even positions used
