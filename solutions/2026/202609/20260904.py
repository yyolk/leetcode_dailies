...
class Solution:
    ...
    * `min(nums[i..n - 1])` is the **smallest** value among the elements
      from index `i` to index `n - 1`.

    An index `i` is called **stable** if its instability score is **less
    than or equal to** `k`.

    Return the **smallest** stable index. If no such index exists, return
    -1.
    ...
    * `0 <= k <= 10^9`
    ...
    def first_stable_index(self, nums: list[int], k: int) -> int:
        n = len(nums)
        # suffix_min[i] = minimum of nums[i..n-1]
        suffix_min = [0] * n
        suffix_min[-1] = nums[-1]
        ...
        for i in range(n):
            ...
            # instability = max of left part - min of right part (incl. i)
