...
class Solution:
    ...
    def max_sum_of_three_subarrays(self, nums: list[int], k: int) -> list[int]:
        ...
        best_three_sequence = [
            ...
            k,
            k * 2,
        ]  # Starting indices of the best three sequences
        ...
        best_three_sum = (
            sequence_sum + sequence_two_sum + sequence_three_sum
        )  # Best sum for three sequences
        ...
        while three_sequence_index <= len(nums) - k:
            ...
            sequence_sum = (
                sequence_sum - nums[sequence_index - 1] + nums[sequence_index + k - 1]
            )
            sequence_two_sum = (
                sequence_two_sum
                - nums[two_sequence_index - 1]
                + nums[two_sequence_index + k - 1]
            )
            sequence_three_sum = (
                sequence_three_sum
                - nums[three_sequence_index - 1]
                + nums[three_sequence_index + k - 1]
            )
