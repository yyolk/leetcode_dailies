    ...
    def leftmost_building_queries(
        ...
        for query_index, query in enumerate(queries):
            ...
            if (
                ...
                or heights[alice_position] < heights[bob_position]
            ):
                result_for_queries[query_index] = bob_position
            else:
                # Add to list for further processing if they can't meet directly
                queries_needing_processing.append(
                    (alice_position, bob_position, query_index)
                )
        ...
        for alice_position, bob_position, query_index in sorted(
            queries_needing_processing, key=lambda x: x[1], reverse=True
        ):
            ...
            while current_building_index > bob_position:
                ...
                while (
                    monotonic_stack
                    and heights[monotonic_stack[0]] < heights[current_building_index]
                ):
            ...
            building_index = bisect_right(
                monotonic_stack, heights[alice_position], key=lambda x: heights[x]
            )

            # Update result if a meeting point is found, otherwise keep -1
            result_for_queries[query_index] = (
                -1
                if building_index == len(monotonic_stack)
                else monotonic_stack[building_index]
            )
